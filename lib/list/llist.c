...
void list_remove_internal(list_t *self, list_node_t *node, bool free_node_memory)
{
  if (node == NULL || (self->head == NULL && self->tail == NULL))
  {
    return;
  }

  list_node_t *a = node->prev;
  list_node_t *c = node->next;
...
list_rpush(list_t *self, list_node_t *node)
{
  ...
  list_remove_internal(self, node, false);
...
{
  return list_remove_internal(self, node, true);
}
